...
EOT;
        ...
        if ($this->fileActionSettings['showDrag']) {
            ...
            $this->script .= <<<EOT
$(".{$formClass} input{$this->getElementClassSelector(false)}").on('filesorted', function(event, params) {
    ...
    $(".{$formClass} input{$this->getElementClassSelector(false)}_sort").val(order);
});
EOT;
        }
    }
